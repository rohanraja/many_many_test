class ChoicesController < ApplicationController
  ...
  # GET /choices/1.json
  def show
    @ch_array = @choice.get_meals #YAML.load(@choice.ch_array)
  ...
  def create
    ...
    @choice.time = DateTime.new(Time.now.year, Time.now.month)

    ch_array = [2,3,4]
    @choice.ch_array = ch_array.to_yaml 
  ...
  private
    ...
    def choice_params
      params.require(:choice).permit(:student_id, :ch_array, :time, :total)
    end
end
